...
//----------------------------------------------------------------
/*
 * 2. dynamic_cast  ->  can only work on pointer or ref. Cannot work on objects
 */
dog* pd = new yellowdog();
yellowdog py = dynamic_cast<yellowdog*>(pd); //!(down cast)
// a. It convert pointer/reference from one type to a //?related type 
 ...
 *
 * Example with dynamic_cast: //! require types to be polymorphic
 */
class dog {
   public:
   virtual ~dog() {}
...
class yellowdog : public dog {
   int age;
   public:
   void bark() { cout<< "woof" <<endl;} // could be treated as static by compiler
   // void bark() { cout<< "woof. I am " << age  << endl; }// cant be treated as static by compiler
};
...
int main() {
   ...
   // py =0
   py->bark(); // compiler interpret bark to be static as bark fn didnt access any member of yellowdog
   cout << "py = " << py << endl;
   cout << "pd = " << pd << endl;
   // ...

   // yellowdog* py = static_cast<yellowdog*>(pd);
   // ! no runtime type check, hence  py will not be nullptr
   // py->bark(); //trying to access mem not belonging to py

   //Soln
   yellowdog* pyd = dynamic_cast<yellowdog*>(pd);
   // py =0
   if(pyd){
      pyd->bark();
      cout << "py = " << pyd << endl;
...
class dog {
	...
	std::string m_name;
	dog():m_name("Bob") {}
	void bark() const {  // using this const fn. makes *this a const
	  std::cout << "My name is " << m_name << std::endl;
     //m_name = "Henry";
   //   const_cast<dog*>(this)->m_name = "Henry"; to make member modification possible within const fn
	}
};
