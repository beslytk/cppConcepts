...
// #define USING 1
// #define KOENIG_LOOKUP 1
// #define NAME_HIDING 1
#define WHY_KOENIG_LOOKUP 1
...
class D : private B { // private inheritance
   public:
    ...
    //   using std::cout;    // illegal use of namespace meber in -> class scope
    // using namespace std;  // illegal 
};
// using B::f; //! illegal -> global scope
D d;
d.f(8);
...
class D : public B { 
   public:
      void f();
    //   using B::f;   // to overcome name hiding
};

int main() {
   D d;
   d.f(8); // B::f() shadowed by D::f() // this is called name-hiding
}
...
namespace {   // Namespace w/o name. Everything within namespace can be accessed within same file
   void g() { std::cout << "g()\n"; }
   void h() { std::cout << "h()\n"; g(); }
}
...
int main() {
   ...
   // wont compile with global g(X) as visibility of 2 g() make it ambiguous
}

//? not only look for g() in current scope and global scope , but also
//? search fn in scope where its parameter type is defined (struct X here)
...
int main() {
   C::Y y;
   h(y);  // Error  // Koenig lookup only applies to namespace
}
...
namespace C {
   ...
   void j() {
      A::X x;
      g(x);   //ambiguous from 2 instance of fn g()
   }
}
int main() {
   C::j();  //compiler err
...
namespace A
{
   ...
   namespace C {
      ...
      void j() {
         // using A::g; // fix for below problem
         g(8);   // wont compile, as A::g(int) is shadowed by C::g()
      }
   }
}
...
namespace A
{
   ...
   namespace C {
      ...
      void j() {
         //using A::g;
         X x;
         g(x); // this will compile bcoz of Koenig lookup for parameter(X here) definition scope
      }
   }
}
...
int main() {
   A::X x1;
   g(x1);   // Koenig Lookup, or Argument Dependent Lookup (ADL)
   g();     // Error as there is no param to help in koenig lookup
}
