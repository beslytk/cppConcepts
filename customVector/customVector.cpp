...
public:
	...
	public:
		...
		explicit iterator(DT* p) // explicit keyword avoid implicit conversion of DT*p to iterator
			: ptr(p)
		{
		}
		...
		DT operator*() const
		{
			return *ptr;		// ret val pointed by iterator
		}
		...
		{
			iterator temp(*this);
			++*this;              // use preIncrement on current Object
			return temp;
		}
	};
...
{
	if (length == capacity) { // no space for new data
		DT* old = arr;
		arr = new DT[capacity = capacity * 2]; // allocate double the existing mem
...
template <typename DT>
DT& GenericMyVector<DT>::operator[](ll index) // reference used for return type to allow assignment
{
...
// Template class to display elements in the vector
template <typename V>
